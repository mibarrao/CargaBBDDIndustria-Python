...
#ruta_excel =  r'C:\Users\mibarra\Downloads\Montos en créditos de consumo otorgados por el sistema C.C.A.F. a afiliadas(os) pensionadas(os).xls'
...
cursor.execute(query_log_success, ('BI_MontosCreditosConsumo', 'PROCESANDO', 'Inicio Carga Excel', '0'))
conexion.commit()
...
try:
    ...
    try:      
        ...
        with engine.begin() as con:
            con.execute(text("DROP TABLE IF EXISTS TMP_MONTO_CREDITOS;"))
            

        excel_file.to_sql(
            name="TMP_MONTO_CREDITOS"
            , con=engine
            , if_exists='replace'
            , index=False
            )
        ...
        cursor.execute(query_log_success, ('BI_MontosCreditosConsumo', 'PROCESADO', 'Ejecución Correcta TMP_MONTO_CREDITOS', cantidadFilas))
        conexion.commit()

        # --- EJECUCION PROCEDURE ---

        # print(f"\nEjecutando procedimiento almacenado...")
        cursor.execute(query_log_success, ('BI_MontosCreditosConsumo', 'PROCESANDO', 'Inicio exec Etl_BaseIndustria_MontosCreditoConsumo', '0'))
        conexion.commit()

        
        spName = 'dbo.Etl_BaseIndustria_MontosCreditoConsumo'
        ...
        cursor.execute(query_log_success, ('BI_MontosCreditosConsumo', 'PROCESADO', 'Ejecución Correcta Etl_BaseIndustria_MontosCreditoConsumo', {cantidadFilas}))
        conexion.commit()

        
    except Exception as e:
        print(f"Error al procesar la carga: {e}")
        cursor.execute(query_log_success, ('BI_MontosCreditosConsumo', 'ERROR', 'Revisar Ejecución en tablas o proceso de insercion', '0'))
        conexion.commit()    


except Exception as e:
    print(f"Error critico al procesar: {e}")
    cursor.execute(query_log_success, ('BI_MontosCreditosConsumo', 'ERROR', 'Revisar Ejecución en archivo de proceso o conexion', '0'))
    conexion.commit()    
