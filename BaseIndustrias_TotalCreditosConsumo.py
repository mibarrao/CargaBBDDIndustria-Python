...
#ruta_excel = r'C:\Users\mibarra\Downloads\Número total de créditos de consumo otorgados por el sistema C.C.A.F_.xls'
ruta_excel = r'C:\Users\mibarra\Downloads\Número total de créditos de consumo otorgados por el sistema C.C.A.F.xlsx'
...
cursor.execute(query_log_success, ('BI_TotalCreditosConsumo', 'PROCESANDO', 'Inicio Carga Excel', '0'))
conexion.commit()
try:
    ...
    try:      
        ...
        with engine.begin() as con:
            con.execute(text("DROP TABLE IF EXISTS TMP_N_CREDITOS;"))
        ...
        cursor.execute(query_log_success, ('BI_TotalCreditosConsumo', 'PROCESADO', 'Ejecución Correcta TMP_N_CREDITOS', {cantidadFilas}))
        conexion.commit()

        # --- EJECUCION PROCEDURE ---

        # # print(f"\nEjecutando procedimiento almacenado...")
        cursor.execute(query_log_success, ('BI_TotalCreditosConsumo', 'PROCESANDO', 'Inicio exec Etl_BaseIndustria_TotalCreditoConsumo', '0'))
        conexion.commit()

        spName = 'dbo.Etl_BaseIndustria_TotalCreditoConsumo'
        with engine.begin() as con:

            con.execute(text(f"EXEC {spName}"))
            cursor.execute(query_log_success, ('BI_TotalCreditosConsumo', 'PROCESADO', 'Ejecución Correcta Etl_BaseIndustria_TotalCreditoConsumo', {cantidadFilas}))
            conexion.commit()
        ...
        cursor.execute(query_log_success, ('BI_TotalCreditosConsumo', 'ERROR', 'Revisar Ejecución en tablas o proceso de insercion', '0'))
        conexion.commit()    


except Exception as e:
    print(f"Error critico al procesar: {e}")
    cursor.execute(query_log_success, ('BI_TotalCreditosConsumo', 'ERROR', 'Revisar Ejecución en archivo de proceso o conexion', '0'))
    conexion.commit()    
