...
ruta_excel = r'C:\Users\mibarra\Downloads\Número total de afiliadas(os) a C.C.A.F.xlsx'
...
cursor.execute(query_log_success, ('BI_CantAfiliados', 'PROCESADO', 'Inicio Carga Excel', '0'))
conexion.commit()
...
try:
    ...
    try:      
        ...
        with engine.begin() as con:
            con.execute(text("DROP TABLE IF EXISTS TMP_CANTAFILIADOS;"))
            

        excel_file.to_sql(
            name="TMP_CANTAFILIADOS"
            , con=engine
            , if_exists='replace'
            , index=False
            )
        ...
        cursor.execute(query_log_success, ('BI_CantAfiliados', 'PROCESADO', 'Ejecución Correcta TMP_CANTAFILIADOS', {cantidadFilas}))
        conexion.commit()

        # --- EJECUCION PROCEDURE ---

        # print(f"\nEjecutando procedimiento almacenado...")
        cursor.execute(query_log_success, ('BI_CantAfiliados', 'PROCESANDO', 'Inicio exec Etl_BaseIndustria_CantidadAfiliado', '0'))
        conexion.commit()

        spName = 'dbo.Etl_BaseIndustria_CantidadAfiliado'
        with engine.begin() as con:

            con.execute(text(f"EXEC {spName}"))

        #     print(f"procedimiento almacenado ejecutado exitosamente.")
        cursor.execute(query_log_success, ('BI_CantAfiliados', 'PROCESADO', 'Ejecución Correcta Etl_BaseIndustria_CantidadAfiliado', {cantidadFilas}))
        conexion.commit()

        
    except Exception as e:
        print(f"Error al procesar la carga: {e}")
        cursor.execute(query_log_success, ('BI_CantAfiliados', 'REVISAR', {e}, 0))
        conexion.commit()        
